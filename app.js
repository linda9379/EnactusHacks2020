...
function initMap() {
  ...
  const map = new google.maps.Map(document.getElementById('map'), {
    zoom: 13,
    center: {lat: 43.777955,  lng: -79.344257},
    styles: mapStyle,
  });
  ...
  const apiKey = 'AIzaSyB-BiUEIvWURdIUxNP8AOgT5YdCnNE9rBs';
  ...
  title.textContent = 'Find the nearest trash bin';
  ...
  autocomplete.addListener('place_changed', async () => {
    ...
    lang = map.getCenter().lng().toString();
    lati = map.getCenter().lat().toString();
...
function calculateAndDisplayRoute(directionsService, directionsRenderer) {
  ...
  directionsService.route(
    {
      origin: { lat: parseFloat(lati), lng: parseFloat(lang)},
      destination: { lat: 43.675511, lng: -79.401938},
